...
## score only supports accuarcy score for now, but with the imbalance in data only F1 score is helpful for now, USE WHEN WE GET BETTER DATASET
# param_grid_ = {'C': [1e-5, 1e-3, 1e-1, 1e0, 1e1, 1e2]}
# bow_search = GridSearchCV(LogisticRegression(), cv=5, param_grid=param_grid_)
# tfidf_search = GridSearchCV(LogisticRegression(), cv=5,
#                                   param_grid=param_grid_)
# bow_search.fit(X_tr_bow, y_tr)
# bow_search.best_score_

#Random forest
def random_forest_classify(X_tr, y_tr, X_test, y_test, description, _C=1.0):
    ...
    print('Test Score with', description, 'features', score,'F1 Score:', f1score,'AUC Score:',auc_score)
    return model

print("\nrandom forest:\n")
model_bow = random_forest_classify(X_tr_bow, y_tr, X_te_bow, y_te, 'bow')
model_tfidf = random_forest_classify(X_tr_tfidf, y_tr, X_te_tfidf, y_te, 'tf-idf')